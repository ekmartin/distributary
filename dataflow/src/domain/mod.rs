...
impl Domain {
    ...
    fn recover_snapshots(&mut self, packet: Box<Packet>) {
    ...
    fn handle_recovery(&mut self, packet: Box<Packet>) {
        let snapshot_id = packet.snapshot_id();
        self.snapshot_id = snapshot_id;
        if self.snapshot_id > 0 {
            self.recover_snapshots(packet);
        }
    ...
    fn snapshot(&mut self, packet: Box<Packet>) {
        let snapshot_id = packet.snapshot_id();
        for (local_addr, _node) in self.nodes.iter() {
        ...
        polling_loop.run_polling_loop(|event| match event {
            ...
            PollEvent::Process(packet) => {
                ...
                if group_commit_queues.should_append(&packet, &self.nodes) {
                    ...
                    let merged_packet =
                        group_commit_queues.append(packet, &self.nodes, self.snapshot_id);
            ...
            PollEvent::Timeout => {
                if let Some(m) =
                    group_commit_queues.flush_if_necessary(&self.nodes, self.snapshot_id)
                {
                    self.handle(m);
                    while let Some(p) = self.inject.take() {
                        self.handle(p);
