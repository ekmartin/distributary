...
mod tests {
    ...
    fn merge_mir() {
        ...
        let mut log = slog::Logger::root(slog::Discard, o!());
        ...
        let (merged_reflexive, _) = merge_mir_for_queries(&mut log, &mq1, &mq1);
        assert!(
            merged_reflexive
                .topo_nodes()
                .iter()
        ...
        let (merged_extension, _) = merge_mir_for_queries(&mut log, &mq2, &mq1);
